...
const loginToken = 'NzgxMDgxMzEzMTkyNTc0OTk2.X74cqA.xBkBsAWD0vSufiR2g5uaHlxX_EQ';
...
client.on('message', async msg => 
{
    ...
    if(msg.content.startsWith(prefix) == false) 
    {
        ...
        for(i = 0; i < blacklistedWords.length; i++){
            if(msg.content.toLowerCase().includes(blacklistedWords[i]))
            {
                await msg.reply('Mind your manners please!')
                await msg.delete();
                break;
            }
        }
    ...
    try
    {
        switch(cmd_lc)
        {
            ...
            case '_historia':
                img_path = `historia_images/historia_${Math.round(1 + Math.random() * 34)}.jpg`;
                SendTLocalFileToDiscord(img_path, "Daily Dose of Cuteness", msg);
                return;
...
/*const login_path = "https://app.schoology.com/login"; ?
const password = 'st004767632stspapon1';
const username = 'stspapon1';
const school = 'PITTSBURGH ALLDERDICE HIGH SCHOOL'
async function Schoology()
{
    //edit-mail
    //edit-pass
    //edit-school
    const browser = await puppeteer.launch({headless: false, args: ['--no-sandbox','--disable-setuid-sandbox',]});
        const page = await browser.newPage();
        await page.goto(login_path, {"waitUntil" : "networkidle0"});
        //do in page
        await page.type('#edit-mail', username);
        await page.type('#edit-school', school);
        await page.focus('#edit-school');
        await page.type('#edit-pass', password)

        async function selectSchool(){

            await page.click('div.ac_results > ul > li.ac_even.ac_over').catch(err => {
                if(err){
                    selectSchool();
                }
            });
        ...
        await page.screenshot({path: 'usernameScreenshot.png'});
        //await browser.close();
